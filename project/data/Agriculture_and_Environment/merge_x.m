...
%rename rows
h = access_to_non_solid_fuel(:,1).Var1;
T.Properties.RowNames = h;

%fill table with all the data
%access to electricity - 2015
for m=1:size(access_to_non_solid_fuel(:,57),1)
    T(m,1) = array2table([access_to_non_solid_fuel{m,57}]);
end
...
for n=3:size(all_tab,1)
    for m=1:size(access_to_non_solid_fuel,1)
...
match = strcmp(co2_emission_kt{:,3},apple_prod{:,1}); %check if countries match.
T2.Properties.RowNames = co2_emission_kt{:,3};
for m=1:size(T2,1)
    match_nan = 1;
    counter_tourism = 19;
    counter_pc = 34;
    ...
    while (match_nan && counter_tourism > 5 )
        if (~iscell( str2double(strrep(co2_emission_kt{m,counter_tourism},',','')) ))
            if (isnan( str2double(strrep(co2_emission_kt{m,counter_tourism},',','')) ))
                counter_tourism = counter_tourism - 1;
                if (counter_tourism == 4), T2(m,n) = table(nan); end
                
            else
                T2(m,1) = array2table(str2double(strrep(co2_emission_kt{m,counter_tourism},',','')));
                match_nan = 0;
                break;
            end
        end
...
tmp_sudan.Properties.RowNames = access_to_non_solid_fuel{114,1};
T2 = [T2;tmp_sudan];
T2(115:end,:) = T2(114:137,:);
T2.Properties.RowNames = access_to_non_solid_fuel{:,1};
T2(114,:) = tmp_sudan;
